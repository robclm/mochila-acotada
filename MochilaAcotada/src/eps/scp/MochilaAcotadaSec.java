...
public class MochilaAcotadaSec {
    ...
    {
        bok = new MochilaAcotada(400, 1, 1); // 400 dkg = 400 dag = 4 kg
    ...
    public MochilaAcotadaSec(int MaxWeight, String InputItemsFile, int nThreads, int M)
    {
        bok = new MochilaAcotada(MaxWeight, nThreads, M);
    ...
    public static void main(String[] args)
    {
        ...
        if(args.length < 3){
            System.out.println("Error params: MochilaAcotada_Sec <input file> <ouput file> [num_threads=N]");
            mochila = new MochilaAcotadaSec();
        }else{
            nThread = Integer.parseInt(args[2]);
            M = Integer.parseInt(args[3]);
            mochila = new MochilaAcotadaSec(Integer.parseInt(args[0]),args[1], nThread, M);
